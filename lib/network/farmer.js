...
FarmerInterface.prototype.join = function(callback) {
  ...
  Network.prototype.join.call(this, function(err) {
    ...
    self.transport.shardServer.on('shardUploaded', function(item){
      if (self._dataReceivedCount < Number.MAX_SAFE_INTEGER) {
        self._dataReceivedCount++;
      } else {
        self._dataReceivedCount = 0;
      }
      self._dataReceivedBytes += item.getContract({
        nodeID: Object.keys(item.contracts)[0]
      }).get('data_size'); // max integer (bytes) is equal to 8PB and additions don't increase it further
    });
